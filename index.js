...
if (fs.existsSync(FILE_NAME)) {
  try {
    lastOrders = JSON.parse(fs.readFileSync(FILE_NAME));
  } catch {
    lastOrders = [];
  }
}
...
function saveOrders() {
  fs.writeFileSync(FILE_NAME, JSON.stringify(lastOrders));
}
    ...
    const response = await axios.get(
      TARGET_URL,
      {
        headers: {
          Cookie: COOKIE,
          Referer: TARGET_URL,
          Origin: "https://watertankcontroller.com",
...
console.log("Smart Order Alert Started...");
